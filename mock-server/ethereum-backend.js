...
var contractABI = [
  ...
  },
  {
    ...
    inputs: [
      ...
      { indexed: false, name: "staking", type: "uint256" }
    ],
    name: "LogTest",
    type: "event"
  },
...
*/
  ...
  for (i = 0; i < nodeAddrs.length; i++) {
    nodeAddr = nodeAddrs[i];
    const node = await contract.methods.nodes(nodeAddr).call();
    selfStakedAmount = Math.round(node.selfStakedAmount / 1000000000000000000);
    totalOtherDelegatedAmount = Math.round(
      node.totalOtherDelegatedAmount / 1000000000000000000
    );
    accumulatedReward = Math.round(
      node.accumulatedReward / 1000000000000000000
    );
    accumulatedRewardRate = node.accumulatedRewardRate;
    running = node.running;
    rewardCut = node.rewardCut;

    //Get from metamask
    delegatorAddr = "0x3E268ECB08CF59B5c2aDBf98651ccD8041C60f67";
    const delegator = await contract.methods
      .delegators(delegatorAddr, nodeAddr)
      .call();
    myDelegator = Math.round(delegator.delegatedAmount / 1000000000000000000);
    myReward = Math.round(delegator.accumulatedReward / 1000000000000000000);
    console.log(
      "self Staked:",
      selfStakedAmount,
      "\n",
